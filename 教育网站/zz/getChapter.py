...
def getVersionData():
    db, cursor = getDB()
    sql = 'select * from data where ok=1'
    cursor.execute(sql)
...
def work():
    ...
    for line in jcData:
        ...
        versions = line[9]
        versionsId = line[10]
        url = 'https://plszz.zzedu.net.cn/youjiao/baceContent.do?section={}&grade={}&volume={}&subject={}&versions={}&listType=1&classHour=1'.format(
            section, grade, volume, subject, versions)
        ...
        for chapter in jsonData[1:]:
            ...
            cursor.execute(sql, (volumeId, gradeId, section,
            ...
            for childChapter in classHours:
                ...
                parentId = code
                sql = 'insert into chapter(termid,gradeid,sectioncode,subjectid,versionid,vc_chapterId,vc_chapterName,vc_material_id,seq_no,parentId) values(%s,%s,%s,%s,%s,%s,%s,%s,%s,%s)'
                cursor.execute(sql, (volumeId, gradeId, section, subjectId,
